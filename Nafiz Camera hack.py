...
# Supported Countries (44 countries from Nafiz camera Hack)
COUNTRIES = {
...
def scan_single_ip_with_detection(ip: str, credentials: List[Tuple[str, str]], ports: List[int]) -> Optional[dict]:
    ...
    Similar to NAfiz camera hack.py logic
    """
    global scanned_count, total_ips, cctv_output_file
...
def scan_country_cameras(country: dict, credentials: List[Tuple[str, str]], max_workers: int = None):
    ...
    Similar to NAfiz camera hack.py but with login validation
    """
...
def print_banner():
    """Display main banner - same style as Nafiz Camera hack.py"""
    banner = f"""
███╗   ██╗ █████╗ ███████╗██╗███████╗
████╗  ██║██╔══██╗██╔════╝██║╚══███╔╝
██╔██╗ ██║███████║█████╗  ██║  ███╔╝ 
...
╚██████╗██║  ██║██║ ╚═╝ ██║███████╗██║  ██║██║  ██║
 ╚═════╝╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝
      ...
      ░▀▀▀░▀▀▀░░▀░░▀░▀░░░▀░▀░▀░▀░▀▀▀░▀░▀
        ...
        >>> Nafiz Camera CCTV Hack <<<

────────────────────────────────────────────

        ⚡ Security Research Interface ⚡
        ⚡ System Scanner Initialized ⚡

────────────────────────────────────────────

        Developer / Credit : Nafiz Sheikh
"""
    print(f"{Fore.RED}{banner}{Style.RESET_ALL}")
    print(f"{Fore.GREEN}[*] Developed by: {Fore.YELLOW}Nafiz Sheikh{Style.RESET_ALL}")
    print(f"{Fore.GREEN}[*] GitHub: {Fore.YELLOW}github.com/nafiz1000x{Style.RESET_ALL}")
